...
//            ref.close();
    ...
    $scope.twitterlogin = function() {
        ...
        ref = window.open(socialloginurl + 'Twitter?returnurl=http://www.ting.com', '_blank', 'location=no');
    ...
    $scope.facebooklogin = function() {
        //        console.log(window.location);
        //        var abc = window.location.origin + window.location.pathname;
        ref = window.open(socialloginurl + 'Facebook?returnurl=http://www.ting.com', '_blank', 'location=no');
        stopinterval = $interval(callAtIntervaltwitter, 2000);
        ref.addEventListener('exit', function(event) {
            $http.post(adminurl + "authenticate").success(authenticatesuccess);
            $interval.cancel(stopinterval);
